...
The first half of this practical deals mostly with vertical profile data of birds, i.e. the bird speed and directions at different altitudes. These profiles are extracted from polar volume data by an automated algorithm (vol2bird). In the second half we will have a look at polar volume data as well.
...
```{r, eval=FALSE}
# define local directory on your machine where to download data for this practical
HOME="~/Documents/radar/testschool/"
# check that the directory exists. If the next statement evaluates to FALSE the directory does not exist, and you need to make it.
file.exists(HOME)
...
You R session is now properly set up
...
Although automated bird quantification algorithms become more and more reliable, distinguishing precipitation from birds remains challenging for algoriths in specific cases. It is therefore important to have the skills to  inspect suspicious profiles. That may help you to identify potential errors of the automated methods, and prevent your from overinterpreting the data.

At C-band, an easy way of doing that is plotting the vertical profile of total reflectivity (quantity DBZH), which includes everything: birds, insects and precipitation. At C-band, precipitation typically has higher reflectivities than birds, and also extends to much higher altitudes.
...
1. Make profile plots for this night (see section 4). Make a plot both for bird density (quantity 'dens') and total reflectivity (quantity `DBZH`, showing birds and precipitation combined). Compare the two plots to visually identify periods and altitude layers with precipitation, and make a guess whether rain affected this migration event.

## 7. Relating profile data to underlying polar volume data
###!!! tot hier gekomen met controle !!!###
We will inspect this precipitation event in more detail by inspecting the polar volume data of 20 Sep 2015 20:00 UTC
```{r, eval=FALSE}
# load the polar volume data
vol.rain=read.pvol("~/git/ENRAM-training/seang_pvol_20150920T200000Z.h5")
# print some information about the polar volume
vol.raing
# print information about the polar scans in this polar volume:
vol.rain$scans
# let's extract the first (lowest elevation scan)
scan.rain = vol.rain$scans[[1]]
...
plot(ppi.rain, param="DBZH")
```
1. The precipitation in this event was convective, showing only few localized but intense thunderstorms. Make also a ppi plot of the radial velocity. How does the texture of the radial velocity of the precipitive areas differ from the areas with biological scattering?

1. Estimate the absolute difference in ground speed of the precipitation and the ground speed of the biological scatterers by visually inspecting the radial velocity plot. Are the biological scatterers birds or insects?
...
1. plot the migration traffic rates for the `ts.night` object (see section 5). What is your biological intepretations of the temporal pattern, specifically in relation to the transition between night and day?
2. Make a time-height plot of the diurnal migration from 27 Sep 2015 4:00 UTC to 7:00 UTC (see section 4).

It should be clear from your last two figures that there are two emersions of birds that coincide with sunrise. We are going to look deeper into the polar volume data from which the vertical profiles were calculated to see what these two emersions might be.
...
# load the file names
files=list.files("~/git/ENRAM-training",pattern="seang_pvol_20150927*",full.names=TRUE)
# plot the file at 4:25 UTC, when the first emersion is present, but the second not yet.
vol1=read.pvol("~/git/ENRAM-training/seang_pvol_20150927T042500Z.h5")
# print some information about the polar volume
vol1
# print information about the polar scans in this polar volume:
vol1$scans
...
# Now we are ready to plot the ppi
# plot the radial velocity:
plot(ppi1, param="VRADH")
# plot the reflectivity factor:
plot(ppi1, param="DBZH")
# even more informative is plotting the data on a google earth map
# first download the background image:
satelliteImage=basemap(ppi1,maptype="satellite")
# then overlay the PPI on the satellite image:
map(ppi1,param="DBZH",satelliteImage)
```

1. Make a series of ppi maps for the 2.5 degree polar scan, and save them to disk. Cycle through the images and try to figure out if you can distinghuish the two emersions, and whether they have a different spatial origin. You can either do that by hand, or - if you feel confident with R - use below example code to make and save the images automatically.

```{r, eval=FALSE}
# first, let's store all the polar volumes we want to plot in a vector:
files=list.files("~/git/ENRAM-training/20150927",pattern="h5$",full.names=TRUE)
...
# this very simple loop only prints the contents of variable 'file'
for (file in files){
  print(file)
}
...
```{r, eval=FALSE}
  files=list.files("~/git/ENRAM-training/20150927",pattern="h5$",full.names=TRUE)
  # let's process the first file in vector files
  file.in=files[1]
  # run vol2bird
  vp=vol2bird(file)
  ...
  # similar to the profile files in the ENRAM repository
  # 
  ...
  # check that we can read the output file:
  vp=readvp(file.out)
```
1. write your own for loop to process a batch of polar volume files. Hint: use a similar appraoch as the for-loop of the previous exercise, in which you processed images.
